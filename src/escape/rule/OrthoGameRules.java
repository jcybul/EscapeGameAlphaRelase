...
public class OrthoGameRules
{
	...
	public interface ORules{
		...
		public boolean oTest(OrthoSquareCoordinate from, OrthoSquareCoordinate to,OrthoGame g);
	}
	...
	static ORules canFly = (from,to,g) -> PieceTypeInitializer.canFly((g.PieceTypes.get(g.b.getPieceAt(from).getName()).getAttributes()));
	static ORules canUnblock = (from,to,g)-> PieceTypeInitializer.canUnblock((g.PieceTypes.get(g.b.getPieceAt(from).getName()).getAttributes()));
	...
	//Rules for Ortho 
	static ORules OrthoOmniRules = (from,to,g) -> ((OrthoBoard)g.b).getLocationType(to) != LocationType.BLOCK && getDistance.oTest(from, to, g)
