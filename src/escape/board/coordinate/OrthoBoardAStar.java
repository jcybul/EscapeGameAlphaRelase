            ...
            for (int y = -1; y < 2;y++) {
                ...
                if ((x != 0 || y != 0)&& 
                  ...
                  // check next position is empty or it can fly 
                    &&
                    (
                    ((b.getPieceAt(OrthoSquareCoordinate.makeCoordinate(this.now.x , this.now.y)))  == null || now.x == xstart && now.y == ystart)
                    ||
                    PieceTypeInitializer.canJump(p) && (
                    		(b.getPieceAt(OrthoSquareCoordinate.makeCoordinate(this.now.x+x,this.now.y+y))) == null && (this.now.parent == null || OrthoSquareCoordinate.makeCoordinate(this.now.parent.x,this.now.parent.y).sameOrthogonal(OrthoSquareCoordinate.makeCoordinate(node.x, node.y)))
                    		||
                    		(b.getPieceAt(OrthoSquareCoordinate.makeCoordinate(now.x, now.y)) == null && ( (node.x != xend && node.y != yend) || (OrthoSquareCoordinate.makeCoordinate(xend, yend).equals(OrthoSquareCoordinate.makeCoordinate(this.now.x + x, this.now.y + y)) && b.getPieceAt(OrthoSquareCoordinate.makeCoordinate(xend, yend)).getPlayer() != b.getPieceAt(OrthoSquareCoordinate.makeCoordinate(xstart, ystart)).getPlayer())))
)	
                    ||
                    // it can fly
                    PieceTypeInitializer.canFly(p) 
                    || ((node.x == xend && node.y == yend) && (b.getPieceAt(OrthoSquareCoordinate.makeCoordinate(xend, yend)) != null) &&(OrthoSquareCoordinate.makeCoordinate(xend, yend).equals(OrthoSquareCoordinate.makeCoordinate(this.now.x + x, this.now.y + y)) && b.getPieceAt(OrthoSquareCoordinate.makeCoordinate(xend, yend)).getPlayer() != b.getPieceAt(OrthoSquareCoordinate.makeCoordinate(xstart, ystart)).getPlayer()))
                    )
